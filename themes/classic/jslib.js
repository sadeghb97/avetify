...
function enterBabe(e){
    var inpName=document.getElementById("txt").value;
    
    if(inpName!="" && (e===null || e.keyCode==13)){
        for(var k=0; len>k; k++){
            if(inpName.toUpperCase()===babes[k].toUpperCase() && !isSelect(ids[k])){
                selected=document.getElementById("sel");
                selected.innerHTML+=('<img src="'+imgs[k]+'" id="'+ids[k]+'" class="selimg" title="'+babes[k]+'" onclick="delBabe(event);">');
                document.getElementById("boob"+ids[k]).remove();
                document.getElementById("txt").value="";
                selArray[ind++]=ids[k];
                document.getElementById("hid").value=selArray.toString();
...
function delBabe(e){
    ...
    document.getElementById("babes").innerHTML+=('<option id="'+id+'">'+name+'</option>');
    selArray[k]=selArray[selArray.length-1];
    selArray.length=--ind;
    document.getElementById("hid").value=selArray.toString();
}
