...
class ModernThemeBadCards {
    public static function printCard($img, $name, $description, $link, $options){
        ...
        if($link || $iconLinkExists) {
            ...
            if($name){
                self::printCardPureTitle($name, $smallerTitle);
            }
            else if($description){
        ...
        else self::printCardPureTitle($name, $smallerTitle);
    ...
    public static function printCardPureTitle($title, $smallerMode = false){
        echo '<div ';
        Styler::startAttribute();
        Styler::closeAttribute();
        HTMLInterface::closeTag();
